...
public class Loop implements Runnable
{
	...
	public void run()
	{
		...
		int loopNumber = Admin.useGUI ? ((Integer)GuiAdmin.getSpinner().getValue()).intValue() : 1;
		boolean isFixedLoop = false;
		if (Admin.useGUI)
			isFixedLoop = GuiAdmin.getFixLoop().isSelected();
		while ((this.loop) && (((isFixedLoop) && (loopNumber > 0)) || (!isFixedLoop)))
		{
			...
			Admin.loopCounter += 1;
			try {
				Thread.sleep(Admin.useGUI ? GuiAdmin.getSlider().getValue() * 1000 : 1000);
			}
